def most_common(lst):
    '''
    Finds the most common value of a list.
...
def find_pam(seq):
    '''
    Finds the PAM index of a given column containing sequences.
...
def extract_koike_yusa(ds, seq_col=5, rep1_col=6, rep2_col=7):
    '''
    Processes Wang/Koike-Yusa dataset and extracts 23-nt and 30-nt sequences.
    Gets the log fold change of both replicates, inverses the sign due to negative selection screening
    (so that higher values mean higher efficiency), and calculates their mean.

    :parameter ds: dataset to analyse i.e., Wang, Koike-Yusa as provided by Xu.
...
def extract_haeussler(ds, data_name, data_col=1, seq_col=3, ext_seq_col=7, eff_col=4):
    ...
    :parameter data_name: the name of the dataset we want to extract.
                          We used: 'shkumatava', 'gagnon', 'varshney', 'teboul', 'Hl60', 'chari2015Train293T', 'morenoMateos'.
    ...
    # Extract the relevant dataset by name ('shkumatava', 'gagnon', 'varshney', 'teboul', 'Hl60', 'chari2015Train293T', 'morenoMateos')
    ...
    for _, row in ds_new.iterrows():
        ...
        elif data_name in ['shkumatava', 'gagnon', 'varshney', 'teboul']:
...
def extract_sgdesigner(ds, seq_col=2, ext_seq_col=3, eff_col=1):
    '''
    Extracts 23-nt and 30-nt sequences with their respective efficiencies from sgDesigner dataset.

    :parameter ds: dataset to analyse i.e., sgDesigner.
    :parameter seq_col: # position of column containing spacer (20-nt) sequences e.g., 2nd column -> 2.
    :parameter ext_seq_col: # position of column containing extended sequences e.g., 3rd column -> 3.
    ...
    # Find starting index and extract the appropriate sequences and their corresponding efficiencies divided by 100
    for _, row in ds.iterrows():
        start_index = row.iloc[ext_seq_col-1].find(row.iloc[seq_col-1])
        seq_23.append(row.iloc[ext_seq_col-1][start_index:start_index + 23])
        seq_30.append(row.iloc[ext_seq_col-1][start_index-4:start_index + 26])
...
def rescale(ds, eff_col=1):
    '''
    Rescales all efficiencies into range [0, 1].
...
def csv_to_fasta(ds, length, seq_col=2):
    '''
    Extracts and writes 20-nt/23-nt/30-nt sequences into a FASTA file.

    :parameter ds: dataset to analyse.
    :parameter length: length of extracted sequence.
    :parameter seq_col: # position of column containing 30-nt sequences e.g., 2nd column -> 2.
    :return: None.
    '''

    assert ds.iloc[:, seq_col-1].str.len().mean() == 30, 'Length of input sequences should be 30 nucleotides.'

    if length == 20:
        fasta = open('20-nt.fasta', 'w+')
        ...
        fasta = open('23-nt.fasta', 'w+')

        for index, row in ds.iterrows():
            fasta.write('>Seq_' + str(index+1) + '\n' +
                        row.iloc[seq_col-1][4:27] + '\n')

        fasta.close()

    elif length == 30:
        fasta = open('30-nt.fasta', 'w+')
        ...
        raise ValueError('Length of output sequences should be 20, 23, or 30 nucleotides.')
